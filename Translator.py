...
def readLangs(lang1, lang2, reverse=False, data_txt='eng-deu.txt'):
    print("Reading lines...")

    # Read the file and split into lines
    lines = open(data_txt, encoding='utf-8').\
        read().strip().split('\n')
...
def prepareData(lang1, lang2, reverse=False, data_txt='eng-deu.txt'):
    input_lang, output_lang, pairs = readLangs(lang1, lang2, reverse, data_txt)
...
def get_dataloader(batch_size, data_txt='eng-deu.txt'):
    input_lang, output_lang, pairs = prepareData('eng', 'deu', False, data_txt)
...
def evaluate(encoder, decoder, sentence, input_lang, output_lang):
    with torch.no_grad():
        ...
        _, topi = decoder_outputs.topk(1)   # max among softmax outputs
        ...
        for idx in decoded_ids:
            if idx.item() == EOS_token:
                ...
                break
            decoded_words.append(output_lang.index2word[idx.item()]) # add max likelihood corresponding word
    return decoded_words, decoder_attn

def loss_on_test(dataloader, encoder, decoder, criterion):
